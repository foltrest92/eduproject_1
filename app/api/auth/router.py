...
async def login(response: Response, email_password: EmailPassword) -> TokensPair:
    ...
    response.set_cookie(key='access_token', value=tokens.access_token, max_age=settings.ACCESS_TOKEN_AGE)
    response.set_cookie(key='refresh_token', value=tokens.refresh_token, httponly=True, max_age=settings.REFRESH_TOKEN_AGE)
    return tokens
